class Company {
    ...
    addEmployee(...params) {
        ...
        if (!this.deprt.hasOwnProperty(department)) {
            this.deprt[department] = [];
        }

        this.deprt[department].push({name, salary, position});
        return `New employee is hired. Name: ${name}. Position: ${position}`;
    }

    bestDepartment() {
        const department = Object.entries(this.deprt).sort((a, b) => this.getAvgSalary(b) - this.getAvgSalary(a))[0];

        const avgSalary =  this.getAvgSalary(department[1]).toFixed(2)
        return `Best Department is: ${department[0]}\n` + `Average salary: ${avgSalary}\n` + department[1].sort((a, b) => {
